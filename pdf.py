...
def process_image(image_file):
    try:
        with tempfile.NamedTemporaryFile(delete=False) as temp_image:
            ...
            report_text = ' '.join(text_results)
    except Exception as e:
        st.error(f"Error processing image: {e}")
        report_text = None
    return report_text

# Define function to process PDF
@st.cache_data(show_spinner="Extracting text from the report...")
...
if report_original is not None:
    ...
    if st.button("Submit",type="secondary"):
        if file_type in ['image/png', 'image/jpeg', 'image/jpg']:
            report_text = process_image(report_original)
        elif file_type == 'application/pdf':
            report_text = process_pdf(report_original)
        else:
                        ...
                        messages=[
                            {
                                ...
                                "content": f"""
                                    ...
                                    1.**Details**:
                                        - Take details of patients such as name, age, gender, test date, test type, doctor name etc. in points if available.
